...
bool isAsciiVowel(char ch) {
  return ch == 'a' || ch == 'i' || ch == 'u' || ch == 'e' || ch == 'o';
}

bool isAsciiConsonant(char ch) {
  return ch >= 'a' && ch <= 'z' && !isAsciiVowel(ch);
}
...
const KanaRule* matchRule(const std::string& text, size_t pos) {
...
bool isRulePrefix(const std::string& text, size_t pos) {
  for (const auto& rule : kRules) {
...
RomajiComposition composeRomaji(const std::string& input, bool final) {
...
std::string romajiToHiragana(const std::string& input) {
  return composeRomaji(input, true).committed;
}
